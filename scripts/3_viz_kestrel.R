...
#add fake date to make graph
Phase1_AvgTemp_Sum <- Phase1_AvgTemp_Sum %>% 
  mutate(Date = as.Date("2021-10-15"))
Phase1_AvgTemp_Sum$DateTime <- paste(Phase1_AvgTemp_Sum$Date, " ", Phase1_AvgTemp_Sum$Time)
Phase1_AvgTemp_Sum$DateTime <- strptime(Phase1_AvgTemp_Sum$DateTime, format="%Y-%m-%d %H%M")

Phase1_AvgTemp_Sum$Phase <- as.factor(Phase1_AvgTemp_Sum$Phase)
Phase1_AvgTemp_Sum$Chamber <- as.factor(Phase1_AvgTemp_Sum$Chamber)

glimpse(Phase1_AvgTemp_Sum)
...
Phase1_AvgTemp_Sum %>%
  ...
  ggplot(aes(x = as.POSIXct(DateTime),
             ...
             color = Kestrel)) +
  geom_point() +
  geom_point(data = Phase1_AvgTemp_Sum %>% filter(Chamber == 1, Heatwave == "no"),
             size = 2,
             color = "black",
             shape = 21) +
  geom_point(data = Phase1_AvgTemp_Sum %>% filter(Chamber == 1, Heatwave == "yes"),
             size = 2,
             color = "red",
             shape = 21) +
  ylim(0, 40) +
  ...
  labs(title = "Chamber 1 (PIPO PIED)\nAverage Daily Temperature Fluctuations",
       color = "Kestrel") +
  geom_text(data = Phase1_AvgTemp_Sum[94, ], 
            label = "Heatwave", color = "red", size = 3, vjust = 2) +
  geom_text(data = Phase1_AvgTemp_Sum[46, ], 
...
Phase1_AvgTemp_Sum %>%
  ...
  ggplot(aes(x = as.POSIXct(DateTime),
             ...
             color = Kestrel)) +
  geom_point() +
  geom_point(data = Phase1_AvgTemp_Sum %>% filter(Chamber == 2, Heatwave == "no"),
             size = 2,
             color = "black",
             shape = 21) +
  geom_point(data = Phase1_AvgTemp_Sum %>% filter(Chamber == 2, Heatwave == "yes"),
             size = 2,
             color = "red",
             shape = 21) +
  ylim(0, 40) +
...
Phase1_AvgTemp_Sum %>%
  ...
  ggplot(aes(x = as.POSIXct(DateTime),
             ...
             color = Kestrel)) +
  geom_point() +
  geom_point(data = Phase1_AvgTemp_Sum %>% filter(Chamber == 3, Heatwave == "no"),
             size = 2,
             color = "black",
             shape = 21) +
  geom_point(data = Phase1_AvgTemp_Sum %>% filter(Chamber == 3, Heatwave == "yes"),
             size = 2,
             color = "red",
             shape = 21) +
  ylim(0, 40) +
