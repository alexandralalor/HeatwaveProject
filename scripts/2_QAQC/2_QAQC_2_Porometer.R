...
Data_Porometer <- merge(Data, Data_Porometer_add_1, by = c("Phase","Species","SpeciesID", "Week"), all = TRUE)
...
#rearrange columns
# Phase, Chamber, Scientific Name, Common Name, Species, SpeciesID,
# Treatment_temp, Treatment_water, PorometerSubset, 
# BasalDia_mm, Height_mm, Whorls, Biomass_g, Week
Data_Porometer <- Data_Porometer[ ,c(1,5,6,7,2,3,8,9,10,11,12,13,14,4,15,16,17,18,19,20,21,22)]

#save as csv
write.csv(Data_Porometer, "data/data_QAQC/Data_Porometer.csv", quote = FALSE, row.names = FALSE)
...
################################################################################
## Below code was used in Phase 1 to QAQC written porometer measurements with  #
##  actual measurements from device records. Not needed to complete analysis  ##
################################################################################



# #read CSVs
# Phase1_Porometer <- read_csv("data/data_QAQC/Phase1_Porometer.csv")
# Data <- read_csv("data/data_QAQC/Data.csv")
# 
